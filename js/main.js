...
6. SCROLL TO CONTENT
7. CONTENT APPEAR ANIMATION INIT / #harvey-details
8. CONTENT CLOSE INITI / #harvey-close-content
9. NEWSLETTER FORM INIT / #harvey-form
...
21. Preloader / .harvey-loading-screen
...
function contentToggle($this){
  ...
  var self    = $this,
    details   = $('#harvey-details'),
    content   = $('#harvey-animate-area'),
    close     = $('#harvey-close-content'),
    hide      = content.data('hide'),
    animation = details.data('animation');

  if ( self.hasClass('harvey-animate-open') ) {
    close.hide(0);
    self.removeClass('harvey-animate-open');
    ...
    self.addClass('harvey-animate-open');
...
--------------------------------------------------------------*/
$(document).on("click", ".harvey-animate-btn", function(e){
  e.preventDefault();
  var self    = $(this);
...
function clickToSlide($this){
  ...
  var self    = $this,
    content   = $('#harvey-details'),
    col   = $('.harvey-col-content'),
    close     = $('#harvey-close-content-slide'),
...
$(document).on("click", ".harvey-content-view", function(e){
  e.preventDefault();
  var self = $(this);
...
--------------------------------------------------------------*/
$(document).on("click", "#harvey-close-content", function(e){

  e.preventDefault();

  var self    = $(this),
    details   = $('#harvey-details'),
    btn       = $('.harvey-animate-btn'),
    content   = $('#harvey-animate-area'),
    close     = $('#harvey-close-content'),
    hide      = content.data('hide'),
    animation = details.data('animation');
    ...
    close.hide();
    btn.removeClass('harvey-animate-open');
    content.removeClass(hide).addClass(animation).addClass('mrs-active');
    details.removeClass(animation).addClass(hide).removeClass('mrs-content-appear').addClass('mrs-conent-disapear');
  
});

$(document).on("click", "#harvey-close-content-slide, .harvey-col-content.mrs-default-content-off", function(e){

  e.preventDefault();

  var self    = $(this),
    details   = $('#harvey-details'),
    btn       = $('.harvey-content-view'),
    close     = $('#harvey-close-content-slide'),
    col   = $('.harvey-col-content'),
    animation = details.data('animation');
...
if ( $("#harvey-form").is_exist() ) {

  var mform = $("#harvey-form");
  mform.ajaxChimp({
  ...
  function callbackFunction (resp) {
      if (resp.result === 'success') {
        ...
        if ( $('.harvey-newsletter-header').length > 0 ) {
          $('.harvey-newsletter-header').removeClass('mform-header-animate').addClass('mfrom-header-animate-close');
        }
        setTimeout(function(){
          mform.removeClass('mform-success').removeClass('mform-submitting').removeClass('mform-animate');
          if ( $('.harvey-newsletter-header').length > 0 ) {
            $('.harvey-newsletter-header').removeClass('mfrom-header-animate-close').addClass('mfrom-header-animate');
          }
...
$(window).on("load resize",function(){
  ...
  if ( $(".harvey-main-content").is_exist() ) {

    var content = $(".harvey-main-content"),
        close   = $('#harvey-close-content');
...
/*--------------------------------------------------------------
  21. Preloader
--------------------------------------------------------------*/
$(".harvey-loading-screen").delay(200).fadeOut('slow');
